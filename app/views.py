...
class ProductCrud(ViewSet):
    def list(self,request):
        ...
        JPO=Serializerproduct(LPO,many=True)
        return Response(JPO.data)
    
    def retrieve(self,request,pk):
        PO=Product.objects.get(pk=pk)
        JPO=Serializerproduct(PO)
        return Response(JPO.data)
    
    def create(self,request):
        JD=request.data
        PDO=Serializerproduct(data=JD)
        if PDO.is_valid():
            PDO.save()
    ...
    def update(self,request,pk):
        PO=Product.objects.get(pk=pk)
        JD=request.data
        PDO=Serializerproduct(PO,data=JD)
        if PDO.is_valid():
            PDO.save()
    ...
    def partial_update(self,request,pk):
        PO=Product.objects.get(pk=pk)
        JD=request.data
        PDO=Serializerproduct(PO,data=JD,partial=True)
        if PDO.is_valid():
            PDO.save()
            return Response({'Update':'Data is Updated'})
        else:
